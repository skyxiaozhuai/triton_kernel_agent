...
- ✅ **主线 A：融合算子家族 add_relu / relu_sum**（借鉴 Fuser 最简版）：单 kernel 融合、中间结果不落全局内存；真机 reference vs golden 全过（add_relu agent 第 1 轮通过, err=0）。`scripts/bench_fused.py` 量化融合 vs 分离：add_relu **1.66x** / relu_sum **2.44x**（GTX1650 主 case 趋势；服务器大 shape 通常更显著）。
