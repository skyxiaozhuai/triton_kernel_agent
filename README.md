...
- **`--model` / 思考默认关**：切 LLM 模型（pro/flash）；**run_opt 默认关思考**（`--thinking` 可开）——复杂 kernel 优化从"卡 40min+ 疯狂空截断"变成 25s 干净完成，token 也降一个量级；run_agent 首次生成默认保持思考（质量优先，可用 `--no-thinking` 关）。
